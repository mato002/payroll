...
@section('content')
    <div class="space-y-6">
        ...
        {{-- Stepper --}}
        <ol class="flex items-center justify-between gap-2 text-xs font-medium text-gray-600 dark:text-gray-300">
            @php
                $steps = [
                    1 => __('Select period'),
            ...
            @foreach($steps as $index => $label)
                ...
                <li class="flex-1 flex items-center">
                    <div class="flex items-center gap-2">
                        ...
                        <span
                            class="@if($state === 'current') text-indigo-600 dark:text-indigo-300 @elseif($state === 'complete') text-emerald-600 dark:text-emerald-300 @endif"
                        >
                            {{ $label }}
                        </span>
                    </div>
                    @if($index < count($steps))
                        <div class="mx-2 h-px flex-1 bg-gray-200 dark:bg-gray-700"></div>
                    @endif
                </li>
            @endforeach
        </ol>
            ...
            <form method="POST" action="{{ route('payroll.runs.wizard.store', ['company' => currentCompany()?->slug]) }}">
                ...
                @elseif ($step === 4)
                    ...
                    <div class="space-y-4">
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400">
                            {{ __('Review the details below and confirm to create and process this payroll run. You will still be able to submit it for approval using the approval workflow.') }}
                        </p>
                        ...
                        <div class="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-800 dark:border-amber-900/60 dark:bg-amber-950 dark:text-amber-200">
                            {{ __('By confirming, the system will calculate payroll for all active employees in the selected period. Any configuration errors (missing salary data, etc.) will be reported.') }}
                        </div>
                    </div>
                @endif
                ...
                <div class="mt-6 flex items-center justify-between">
                    <div>
                        @if($step > 1)
                            <a
                                ...
                                class="inline-flex items-center rounded-md border border-gray-200 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 shadow-sm hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 dark:hover:bg-gray-800"
                            >
                                {{ __('Back') }}
                            </a>
                        @endif
                    ...
                    <div class="flex items-center gap-2">
                        @if($step < 4)
                            <button
                                type="submit"
                                class="inline-flex items-center rounded-md bg-indigo-600 px-4 py-1.5 text-xs font-medium text-white shadow-sm hover:bg-indigo-700"
                            >
                                {{ __('Next') }}
                            </button>
                        @else
                            <button
                                type="submit"
                                class="inline-flex items-center rounded-md bg-emerald-600 px-4 py-1.5 text-xs font-medium text-white shadow-sm hover:bg-emerald-700"
                            >
