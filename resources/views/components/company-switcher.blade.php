        ...
        >
            <div class="py-1">
                @foreach($companies as $company)
                    <form action="{{ route('companies.switch.store', $company) }}" method="POST" class="block">
                        @csrf
                        <input type="hidden" name="redirect_to" value="{{ url()->current() }}">
                        <button 
                            type="submit"
                            class="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 transition-colors flex items-center justify-between
                            {{ $currentCompany && $currentCompany->id === $company->id ? 'bg-blue-50 text-blue-700' : 'text-gray-700' }}"
                        >
                ...
                <div class="border-t border-gray-200 mt-1">
                    <a href="{{ route('companies.switch.index') }}" 
